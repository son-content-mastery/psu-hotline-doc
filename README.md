...
Local password-reset email uses Django's console backend and appears in `docker compose logs backend`. If `FRONTEND_PORT` changes, also set `FRONTEND_BASE_URL` to the browser URL so reset links are correct. A deployed environment needs a real approved email backend.
